...
int ng_flow_table_init()
{
	...
	struct ng_tcp_flow ** flows = rte_malloc("ng_tcp_flow",sizeof(struct ng_tcp_flow *) * FLOW_TABLE_SIZE,0);
    if (!flows) {
...
{
    struct ng_tcp_flow  * flow = rte_malloc("tcp_flow",sizeof(struct ng_tcp_flow),0);	
    if (!flow) {
        printf("Error allocating memory for new flow.\n");
        return NULL;
    }
	struct ng_tcp_flow_key  * flow_key = rte_malloc("tcp_flow_key",sizeof(struct ng_tcp_flow),0);	
	...
	flow->rcvbuf = rte_ring_create("rcvbuf",RING_SIZE,rte_socket_id(),0);//rte_socket_id保证申请的空间和当前线程在同一内存池
...
bool flow_table_remove(struct ng_flow_table * flow_table, struct ng_tcp_flow * flow)
{
    ...
    flow_table->half_connect_flows[i] = flow_table->half_connect_flows[--flow_table->num_flows];
	rte_free(flow);
	rte_free(flow->key);
    return true;
}
...
bool flow_table_hash_add(struct ng_flow_table * flow_table, struct ng_tcp_flow * flow)
{
    ...
    // 将流对象加入哈希表
    int ret = rte_hash_add_key_data(flow_table->total_connect_flows, &flow->key, flow);
    if (ret < 0) {
...
struct ng_tcp_flow * flow_table_hash_lookup(struct ng_flow_table  * flow_table, uint32_t src_ip, uint32_t dst_ip,uint16_t sport,uint16_t dport)
{
    
}

static struct ng_tcp_flow * ng_tcp_flow_search(uint32_t sip,uint32_t dip,uint16_t sport,uint16_t dport){
    ...
    int ret = rte_hash_lookup_data(tcb_table->total_connect_flows, &key, (void **) &flow);
    if (ret != 0) {
        return flow;
    }
	...
	ret = rte_hash_lookup_data(tcb_table->total_connect_flows, &key, (void **) &flow);
	if (ret != 0) {
		return flow;
	}
	return NULL;
...
static int ng_tcp_handle_listen(struct ng_tcp_flow *flow,struct rte_tcp_hdr*tcphdr,struct rte_ipv4_hdr *iphdr){
	if(tcphdr ->tcp_flags & RTE_TCP_SYN_FLAG){ //RTE_TCP_SYN_FLAG表示发起连接请求
		//TCP中的第一次握手，回发SYN+ACK(第二次握手)
		if(flow->status == NG_TCP_STATUS_LISTEN){ //只有在LISTEN状态才处理，然后切换状态，这样同一个连接只会处理一次请求

			
			struct ng_tcp_flow * syn = flow_create(iphdr->src_addr,iphdr->dst_addr,tcphdr->src_port,tcphdr->dst_port);
...
static int ng_tcp_handle_syn_rcvd(struct ng_tcp_flow * flow,struct rte_tcp_hdr *tcphdr){
	if(tcphdr->tcp_flags & RTE_TCP_ACK_FLAG){
		//TCP中的第三次握手，收到ACK
		if(flow->status == NG_TCP_STATUS_SYN_RCVD){
			uint32_t acknum = ntohl(tcphdr->recv_ack);
			if(acknum == flow->snd_nxt +1){ //表示对端收到了序列号为flow->snd_nxt的数据包
				
			}
			//tcp 连接完成。状态切换为ESTABLISHED
			flow->status = NG_TCP_STATUS_ESTABLISHED;
			//accept
			struct ng_tcp_flow * listener = ng_tcp_flow_search(0,0,0,flow->key->dport);
			if(listener == NULL){
...
static int ng_tcp_enqueue_recvbuffer(struct ng_tcp_flow * flow,struct rte_tcp_hdr *tcphdr,int tcplen){
	//recv buffer
...
static int ng_tcp_send_ackpkt(struct ng_tcp_flow *flow,struct rte_tcp_hdr *tcphdr){
...
static int ng_tcp_handle_established(struct ng_tcp_flow *flow,struct rte_tcp_hdr *tcphdr,int tcplen){
...
static int ng_tcp_handle_close_wait(struct ng_tcp_flow * flow,struct rte_tcp_hdr *tcphdr){
...
static int ng_tcp_handle_last_ack(struct ng_tcp_flow *flow, struct rte_tcp_hdr *tcphdr) {
	if(tcphdr->tcp_flags & RTE_TCP_ACK_FLAG){
...
int ng_tcp_process(struct rte_mbuf *tcpmbuf){
	...
	struct ng_tcp_flow *flow = ng_tcp_flow_search(iphdr->dst_addr,iphdr->src_addr,
...
int ng_tcp_out(struct rte_mempool *mbuf_pool) {
	...
	for(flow = table->tcb_set;flow !=NULL;flow = flow->next){
		...
		if(dstmac == NULL){
			...
			struct rte_mbuf *tcpbuf = ng_tcp_pkt(mbuf_pool,flow->dip,flow->sip,flow->localmac,dstmac,fragment);
